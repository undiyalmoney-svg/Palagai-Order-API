...
async function health(_req, res) {
  ...
  res.json({
    ...
    note: 'Trade Bot paper/live: Nifty + Bank S/R. Up to two ATM CE/PE per book per day, Nifty TIME 6 / Bank TIME 8 unless rupee stop. No stall give-up. Bank skips CE below the day open and PE above it. Not a straddle. Paper ₹ is CE/PE × lot. Live MIS. Day ±₹3,500. Crude off.',
    version: APP_VERSION,
    appBuild: APP_BUILD,
  ...
  if (body.liveMoney === true || body.liveMoney === 'true') {
    try {
      ...
      res.json({
        ...
        note:
          'Crude Bot live buys one ATM Crude Mini PE after NSE close (session OR ≤60 pts, confirm, SL30/TP80, 16:00–19:00, max 2/day). Afternoon CE is off. It does not touch Nifty or Bank.',
      });
    } catch (err) {
