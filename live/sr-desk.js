...
const { announcerFromDesk } = require('./sr-announcer');
const {
...
async function runSrDesk({ authorization, fromDate, toDate, capitalRs, capitalSource, liveMoney }, deps = {}) {
  ...
  return {
    ...
    announcer: announcerFromDesk({ books: booksOut.filter((b) => b.chart).map((b) => b.chart) }),
    play: 'S/R → breakout → confirm (retest) → enter ATM CE/PE',
    note:
      'This desk trades only Nifty 50 and Bank Nifty. S/R → breakout → confirm (retest) → enter ATM CE/PE. Same play on Paper and Live. Up to two ATM CE or PE per book per day (qty 65 / 30, MIS). Nifty holds ~30 minutes (6×5m TIME); Bank holds ~40 minutes (8×5m TIME) unless the rupee stop hits first (Nifty ₹5,000 / Bank ₹3,500). Two 5m diagrams mark support, resistance, breakout, confirm, and entry. The box is still drawn; this DNA does not flatten on that measured-move. Not a stall give-up. Bank does not buy CE below the day\'s open or PE above it. Not FAIL on a 1-bar close through the wall, not a +20 index TARGET. Paper ₹ shadows Live: ATM CE/PE premium × lot, minus ₹20/lot. In/Out are NSE 5-minute option OHLC when the range is ≤14 days, otherwise modeled weekly premium. Crude stays off.',
