...
assert.strictEqual(dna.dailyTargetRs, 0, 'no profit lock — winners run');
assert.strictEqual(dna.strictDayStop, true);
assert.strictEqual(ops.deskGreenLockRs, 0, 'no target lock');
assert.strictEqual(ops.crudeOnlyBelowBand, false, 'crude keeps trading');
