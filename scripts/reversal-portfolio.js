 ...
 */
const REVERSAL = true; // buy the LOSER quintile (short-term reversal), not winners
...
const UNIVERSE = {
  ...
  NESTLEIND: 4598529,
  // --- SURVIVORSHIP-BIAS STRESS SET ---
  // Large/liquid names that suffered severe, permanent-looking declines. A
  // loser-buying strategy is uniquely exposed to survivorship bias: the real
  // universe contained falling knives that never recovered. These are still
  // listed (truly delisted names cannot be fetched at all), so this only
  // PARTIALLY corrects the bias — it makes the test harder, not unbiased.
...
function runMomentum(dates, px, symbols, cfg, niftyClose) {
  ...
  for (let m = 0; m < monthStarts.length; m += 1) {
    ...
    if (regimeOk) {
      ...
      scored.sort((x, y) => (REVERSAL ? x.mom - y.mom : y.mom - x.mom));
      target = scored.slice(0, topN).map((x) => x.s);
    }
...
async function main() {
  ...
  const configs = [
    { name: 'REVERSAL 20d losers, top3, 1mo', topN: 3, holdMonths: 1, lookback: 20, skip: 0, regimeFilter: false },
    { name: 'REVERSAL 20d losers, top5, 1mo', topN: 5, holdMonths: 1, lookback: 20, skip: 0, regimeFilter: false },
    { name: 'REVERSAL 10d losers, top3, 1mo', topN: 3, holdMonths: 1, lookback: 10, skip: 0, regimeFilter: false },
    { name: 'REVERSAL 60d losers, top3, 1mo', topN: 3, holdMonths: 1, lookback: 60, skip: 0, regimeFilter: false },
    { name: 'REVERSAL 20d losers, top5, 2mo', topN: 5, holdMonths: 2, lookback: 20, skip: 0, regimeFilter: false },
  ];
