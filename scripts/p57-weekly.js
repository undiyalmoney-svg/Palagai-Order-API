/** Avg weekly profit per instrument: break with-trend + target, hold losers to
 *  close, MINUS a round-trip cost per trade. Weekly buckets. args: FILE ENTRY TARGET LOT COST eEnd sqOff */
const fs=require('fs');const P=5,TREND_N=20;
const FILE=process.argv[2],ENTRY=+process.argv[3],TARGET=+process.argv[4],LOT=+process.argv[5],COST=+process.argv[6],eEnd=process.argv[7],sqOff=process.argv[8];
...
function weekKey(d){const dt=new Date(d);const day=(dt.getUTCDay()+6)%7;const mon=new Date(dt);mon.setUTCDate(dt.getUTCDate()-day);return mon.toISOString().slice(0,10);}
const wk={};let res=null,sup=null;
for(let i=0;i<B.length;i++){const j=i-P;if(j>=0){if(isPH[j])res=B[j].h;if(isPL[j])sup=B[j].l;}
  ...
  const rs=(pts-COST)*LOT;const k=weekKey(b.d);wk[k]=(wk[k]||0)+rs;}
const weeks=Object.values(wk);const green=weeks.filter(x=>x>0).length;
weeks.sort((a,b)=>a-b);
const mean=a=>a.reduce((x,y)=>x+y,0)/a.length;
const name=FILE.split('/').pop().replace('5m.json','').replace('.json','');
console.log('  '+name.toUpperCase().padEnd(11)+' ('+weeks.length+' weeks, lot '+LOT+', cost '+COST+'pt/trade):');
console.log('    AVG weekly profit: Rs'+Math.round(mean(weeks))+'/lot   |   green weeks '+green+'/'+weeks.length+' ('+(100*green/weeks.length).toFixed(0)+'%)');
console.log('    typical week: Rs'+Math.round(weeks[Math.floor(0.25*weeks.length)])+' to Rs'+Math.round(weeks[Math.floor(0.75*weeks.length)])+'   |   best +Rs'+Math.round(weeks[weeks.length-1])+'  worst Rs'+Math.round(weeks[0]));
